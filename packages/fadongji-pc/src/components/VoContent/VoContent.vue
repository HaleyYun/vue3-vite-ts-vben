<template>
  <div class="flex items-center">
    <div>
      <div @click.stop class="w-60px h-60px" v-if="source && sourceType === 1">
      ...
      <div v-if="source && sourceType === 2" @click="visibleVideo = true">
        <div class="w-60px h-60px rounded-5px overflow-hidden relative">
          ...
          <div
            class="absolute top-18px left-17px z-10 bg-[#fff] rounded-50px text-0 w-25px h-25px flex justify-center items-center"
          >
            <PlayCircleOutlined style="color: #000000" class="text-green-600 text-25px" />
          </div>
        </div>
      </div>
      <div
    ...
    <div class="pl-10px">
      <div v-if="title" class="text-blue-600 text-14px leading-snug
