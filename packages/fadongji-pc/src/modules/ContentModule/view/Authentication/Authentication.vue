      ...
      <EraTableTitle :total="paginations.total">
        <template #tableEdit>
          <AButton
            ...
            v-auth="'/v1/content/certificationCourse/batch/update'"
          >
            批量删除
          </AButton>
          <!-- v-auth="'/v1/content/certificationCourse/category/condition'" -->
          <AButton
            type="primary"
            class="margin-l-r-10"
            @click="addDetails"
        ...
        >
          <template #bodyCell="{ column, text, record }">
            ...
            <template v-if="column.dataIndex === 'operation'">
              ...
              >
                <AButton type="link" v-auth="'/v1/content/certificationCourse/delete'"
                  >删除
                </AButton>
              </a-popconfirm>
            </template>
...
<script lang="ts" name="Authentication" setup>
  import { ref, reactive, onBeforeMount, toRefs, unref, computed } from 'vue'
  ...
  // /v1/content/certificationCourse/enable
  const go = useRouter()
  //编辑
  const editDetails = (record) => {
    ...
    go.push({
      path: '/contentModule/content/curriculumContent',
      query: {
        id: record.id,
      },
    })
  ...
  // 判断是否有权限
  const isHasEnable = computed(() => {
    return !hasPermission('/v1/content/certificationCourse/enable')
  })
  //新增/
  const addDetails = () => {
    go.push({
  ...
  const goClassification = () => {
    go.push({
      path: '/contentModule/content/curriculumClassification',
    })
  }
  ...
  const eraModalOk = async () => {
    ...
    try {
      ...
      const { code, message } = await adminHttp.CONTENT.contentCertificationCourseBatchUpdate(
        params,
      )
      if (code === '20001') {
  ...
  const stateEnable = (record) => {
    ...
    adminHttp.CONTENT.contentCertificationCourseEnable(changeSwitchMap)
      .then(() => {
        loadTableData()
      })
      .catch((error) => {
  ...
  const loadTableData = async () => {
    try {
      ...
      const { code, data } = await adminHttp.CONTENT.contentCertificationCourseCondition(
  ...
  const handlerDeletedRow = (item) => {
    ...
    adminHttp.CONTENT.contentCertificationCourseDelete({ id: item.id })
      .then(() => {
