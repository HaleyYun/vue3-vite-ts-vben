      ...
      <EraTableTitle :total="paginations.total">
        <template #tableEdit>
          <AButton
            ...
            v-auth="'/v1/content/technical/batch/update'"
          >
            批量删除
          </AButton>
          <!-- v-auth="'v1/content/technical/category/condition'" -->
          <AButton type="primary" class="margin-l-r-10" @click="addDetails" v-auth="'/v1/content/technical/save'">新增内容</AButton>
          <AButton type="default" @click="goClassification" v-auth="'v1/content/technical/category/condition'">管理分类</AButton>
        </template>
      </EraTableTitle>
      <div>
        <ATable
        ...
        >
          <template #bodyCell="{ column, text, record }">
            ...
            <template v-if="column.dataIndex === 'modelName'">
             ...
             <div v-if="record.modelName === 'null'"></div>
             <div v-else>{{ record.modelName }}</div>
            ...
            <template v-if="column.dataIndex === 'operation'">
              <AButton type="link" @click="editDetails(record)" v-auth="'/v1/content/technical/category/condition'">编辑</AButton>
              ...
              >
                <AButton type="link" v-auth="'/v1/content/technical/delete'">删除</AButton>
              </APopconfirm>
            </template>
          </template>
        </ATable>
...
<script lang="ts" name="RepairTechnology" setup>
  ...
  const editDetails = (record) => {
    ...
    go.push({
      path: '/contentModule/content/repairContent',
      query: {
        id: record.id,
      },
    })
  ...
  const goClassification = () => {
    go.push({
      path: '/contentModule/content/repairClassification',
    })
  }
  ...
  const eraModalOk = async () => {
    ...
    try {
      ...
      const { code, message } = await adminHttp.CONTENT.contentTechnicalBatchUpdate(params)
      if (code === '20001') {
  ...
  const loadTableData = async () => {
    try {
      ...
      ///vocen-jarvis/v1/content/technical/condition
      const { code, data } = await adminHttp.CONTENT.contentTechnicalCondition(
  ...
  const handlerDeletedRow = (item) => {
    ...
    adminHttp.CONTENT.contentTechnicalDelete({ id: item.id })
      .then(() => {
  ...
  const stateEnable = (record) => {
    ...
    adminHttp.CONTENT.contentTechnicalEnable(changeSwitchMap)
      .then(() => {
        loadTableData()
      })
      .catch((error) => {
  ...
  const selectedEnable = (record) => {
    ...
    adminHttp.CONTENT.contentTechnicalRecommend(switchMap)
      .then(() => {
        loadTableData()
      })
      .catch((error) => {
