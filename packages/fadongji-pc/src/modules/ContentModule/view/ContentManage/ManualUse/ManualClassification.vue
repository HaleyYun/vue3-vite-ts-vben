<template>
  ...
  <div class="mx-24px mt-40px mb-24px">
    <div class="my-24px bg-white p-24px pb-4px">
      <ClassForm @rest="handlerRest" @submit="handlerSubmit" />
    </div>
        ...
        >
          <template #bodyCell="{ column, record }">
            <template v-if="column.dataIndex === 'platformCode'">
              ...
              <span v-if="record.platformCode == 'agent'">修理厂</span>
              <span v-if="record.platformCode == 'supplier'">修理厂</span>
              <span v-if="record.platformCode == 'channelManager'">修理厂</span>
            </template>
            <template v-if="column.dataIndex === 'operation'">
              <AButton type="link" @click="addClassification(record, alertEnum.Edit)">编辑</AButton>
    ...
    <div class="p-5">
      ...
      >
        ...
        <AFormItem name="select" label="所属角色">
          <a-select v-model:value="formState.select" placeholder="">
            ...
            <a-select-option value="op">运营平台</a-select-option>
          </a-select>
        </AFormItem>
...
<script setup lang="ts">
  ...
  import ClassForm from './component/ClassForm.vue'
  ...
  // //删除列表/v1/content/help/category/delete
