...
<script lang="ts" setup>
  ...
  const goOrder = (record) => {
    ...
    router.push({
      path: '/OrderModule/agentOrder/agentAfterSalesOrder',
      query: { buyerName: record.buyerName, brandId: record.brandId, shopId: record.buyerId },
    })
  }
  ...
  const loadTableData = async () => {
    try {
      ...
      const { code, data } = await AdminHttp.ORDER.orderCountInvestRefundCount(params)
