  ...
  <div class="mx-24px  mb-24px">
    ...
    <div class="p-24px bg-white">
      <EraTableTitle :total="paginations.total">
        <template #tableEdit>
          <a-button type="primary"  class="margin-l-r-10" @click="asssociated">关联修理厂</a-button>
        </template>
      </EraTableTitle>
      <div >
        <ATable
    ...
    <AssociatedGrage @register="register" @update="loadTableData()"></AssociatedGrage>
...
<script lang="ts" setup>
  ...
  const asssociated = () =>{
    openModal(true, {id: route.query['id'],bindingType:3})
  }
  ...
  const loadTableData = async () => {
    try {
      ...
      const {code, data} = await AdminHttp.COMPANY.companyInfoSkilledWorkerAssociatedGaragePage(params);
      if (code === '20001') {
