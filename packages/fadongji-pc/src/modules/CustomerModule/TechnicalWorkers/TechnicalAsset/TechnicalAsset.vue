        ...
        >
          <template #bodyCell="{ column, text, record }">
            ...
            <template v-if="column.dataIndex === 'growth_value'">
              <a @click="assetsReport(record)">{{ record.growth_value }}</a>
            </template>
          </template>
        </ATable>
      </div>
...
<script lang="ts" setup>
  ...
  // 资产明细
  const assetsReport = async (record) => {
    router.push({ path: '/customerModule/TechnicalWorkers/Report', query: { id: record.id } })
  }
