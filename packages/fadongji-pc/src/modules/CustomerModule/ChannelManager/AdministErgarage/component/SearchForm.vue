<template>
  ...
  >
    <a-row>
      <a-col :span="colSpan">
        <a-form-item :label="keyword+'名称'" class="mb-24px">
          <a-input v-model:value="formState.garageCompanyName" placeholder="名称"></a-input>
        </a-form-item>
      </a-col>
      <a-col :span="colSpan">
        <a-form-item :label="keyword+'手机号'" class="mb-24px">
          <a-input v-model:value="formState.userName" placeholder="手机号"></a-input>
        </a-form-item>
      </a-col>
      <a-col :span="colSpan">
      ...
      <a-col :span="colSpan" :offset="offset">
        <a-form-item class="mb-24px mr-0 text-right	 " :wrapperCol='{span: 24}'>
          ...
          <a-button style="margin-left: 10px" @click="handlerRest">重置</a-button>
        </a-form-item>
      </a-col>
    </a-row>
...
<script>
  ...
  export default {
    ...
    methods: {
      handlerSubmit() {
        if (this.searchTime.length > 0) {
          this.formState.startTime = this.searchTime[0]+' 00:00:00';
          this.formState.endTime = this.searchTime[1]+' 23:59:59';
        }
        if(this.formState.areaCode &&this.formState.areaCode.length > 0){
          this.formState.areaCode =this.formState.areaCode[this.formState.areaCode.length -1]
        }
    ...
    mounted() {
      this.formState = _.cloneDeep(initialForm);
      this.keyword = this.route.query['type'] == 'garage'?'修理厂':'服务商'
      this.getArea()
    },
  }
</script>
