  ...
  <div class="p-20px  bg-white  mb-82px">
    <div class="w-600px mx-auto">
      <a-alert
              ...
              type="error"
              v-if="failureStatus == 1 || failureStatus == 3"
              message="营业执照审核失败"
              show-icon
      />
      <a-form
      ...
      >
        <a-form-item
                ...
                name="mobile"
                :rules="
          !isDisabled('mobile') ? [{ required: true, validator: checkMobile, trigger: 'blur' }] : []
        "
        >
          <a-input
        ...
        >
          <FormOcRUpload
                  :disabled="isDisabled('businessLicenseUrl')"
                  v-model:value="formState.businessLicenseUrl"
          ...
          <div class="flex justify-between items-baseline">
            ...
            <div>
              ...
              >
                ...
                />
                <a v-if="editType == 'edit'" class="ml-5px" @click="editEndTime('close')">取消</a>
              </a-form-item>
              <div v-else>
                <span class="text-gray-300 mr-20px">{{ formState.businessLicenseEndDate }}</span>
                <form-outlined v-if="editType == 'close'" @click="editEndTime('edit')" />
              </div>
            </div>
          </div>
        </a-form-item>
        <a-form-item
                label="企业LOGO"
                name="storeUrl"
                :rules="[{ required: true, message: '请上传企业LOGO', trigger: 'blur,change' }]"
        >
          <FormLocalUpload :disabled="isDisabled()" v-model:value="formState.storeUrl" />
        </a-form-item>
      </a-form>
    </div>
  </div>
  <div class="fixed bg-white left-250px right-30px bottom-0 flex items-center justify-center p-15px">
    <div>
      <a-space>
...
<script setup lang="ts">
  ...
  const defaultFormState = WebStorage.get('OneStepFormAddBuyer') || {
    mobile: '', // 手机号码
  ...
  const isDisabled = computed(() => {
    return function (key) {
      ...
      if (route.query && route.query.id && key === 'mobile') {
        return true
      }
  ...
  // 下一步
  const nextHandler = () => {
    WebStorage.set('OneStepFormAddBuyer', formState)
  ...
  let checkMobile = async (_rule: Rule, value) => {
    ...
    await AdminHttp.COMPANY.companyLegalOldMachineMobile({ mobile: value })
