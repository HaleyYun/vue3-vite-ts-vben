        ...
        >
          <template #bodyCell="{ column, text, record }">
            <template v-if="column.dataIndex === 'growth_value'">
              <a  @click="assetsReport(record,2)">{{record.growth_value}}</a>
            </template>
          </template>
        </ATable>
      </div>
...
<script lang="ts" setup>
  ...
  const assetsReport = async (record,type) => {
    router.push({path:'/customerModule/OldMachineBuyers/BuyersAsset/BuyersAssetReport',query:{id: record.id,type:type}})
  };
