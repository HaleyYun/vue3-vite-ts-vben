      ...
      <!-- <EraSearchTable :item-width="246" :search-data="searchData">
        ...
        <template #content = "{ row }">
          <template v-if="row.id='supplierGrowthValue002'">
            <Select class="form-select" v-model:value="row.value" :options="options" :placeholder="row.placeHolder" />
          </template>
        </template>
      </EraSearchTable> -->
...
<script lang="ts">
  ...
  // “成长值设置” -- table
  import GrowthValueSettingTable from '/@/modules/CustomerModule/SupplierModule/SuppierGrowthValue/child/GrowthValueSettingTable.vue'
  // “成长值设置” - 查询条件
