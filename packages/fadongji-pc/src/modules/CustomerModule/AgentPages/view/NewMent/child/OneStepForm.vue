...
<script setup lang="ts">
  ...
  const defaultFormState = WebStorage.get('OneStepFormSupplier') || {
    companyType: 2, // 企业类型
    mobile: '', // 手机号码
  ...
  const nextHandler = () => {
    formRef.value
      .validate()
      .then(() => {
        WebStorage.set('OneStepFormSupplier', formState)
  ...
  let checkMobile = async (_rule: Rule, value) => {
    ...
    await AdminHttp.CUSTOMER.apiCompanyLegalAgentMobile({ mobile: value })
