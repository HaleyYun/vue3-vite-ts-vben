<template>
  <div class="p-20px bg-white mb-82px">
    <div class="w-600px mx-auto min-h-[70vh]">
      <a-form
        :model="formState"
        name="formRef"
        :label-col="{ span: 5 }"
  ...
  >
    <div>
      <a-space>
        ...
        <a-button :disabled="disabled" :loading="loading" type="primary" @click="nextHandler">
          提交
        </a-button>
      </a-space>
    </div>
...
<script setup lang="ts">
  ...
  const oneForm = WebStorage.get('OneStepFormSupplier')

  const formRef = ref()
  const isLongDate = computed(() => formState.legalEndDate === '长期')
  const dateFormat = 'YYYY-MM-DD'
  const defaultFormState = WebStorage.get('TwoStepFormSupplier') || {
    legalId: '', // 法人身份证号
  ...
  const nextHandler = () => {
    ...
    formRef.value
      ...
      .then(async () => {
        loading.value = true
        const ret = await AdminHttp.CUSTOMER.apiCompanyLegalAgent(postForm)
        loading.value = false
        if (+ret.code === 20001) {
          WebStorage.remove('OneStepFormSupplier')
          WebStorage.remove('TwoStepFormSupplier')
          message.success('新增成功')
          $router.push('/customerModule/agent/manage')
        }
      })
      .catch((error) => {
        console.log(error)
  ...
  const prevHandler = () => {
    WebStorage.set('TwoStepFormSupplier', formState)
