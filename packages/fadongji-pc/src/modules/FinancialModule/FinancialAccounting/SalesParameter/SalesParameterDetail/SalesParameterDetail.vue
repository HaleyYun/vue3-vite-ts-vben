<template>
  <div class="bg-white h-100vh m-24px p-24px overflow-y-auto">
    <div>
      <div class="template--title mb-20px">支付明细</div>
      <div class="p-20px">
...
<script lang="ts" setup>
  ...
  const loadTableData = async () => {
    try {
      ...
      let params = {
        ...paginations,
        payId: route.query.id as string,
      }
      const { code, data } = await AdminHttp.FINANCE.financePayDetail(params)
      if (code === '20001') {
        payInfo.value = data
        tableData.value = data.orderPage ? data.orderPage.records : []

        paginations.total = data.orderPage.total
      }
