<template>
  <div class="orderTable my-15px">
    <div class="orderHead p-16px">
      <div class="lift">
        <span
          >订单：<a @click="goOrderDetail(tabData)">{{ format(tabData.id) }}</a></span
        >
      </div>
    </div>
    <a-table
    ...
    >
      <template #bodyCell="{ column, text, record }">
        ...
        <template v-if="column.dataIndex == 'amount_of'">
          <div>{{ record.goodsCount * record.payPrice }}元</div>
        </template>
        <!--卖家-->
...
<script lang="ts" setup>
  ...
  watch(
    () => props.tabData,
    (val) => {
      let length = val['goodsDetail'] && val['goodsDetail'].length ? val['goodsDetail'].length : 0
