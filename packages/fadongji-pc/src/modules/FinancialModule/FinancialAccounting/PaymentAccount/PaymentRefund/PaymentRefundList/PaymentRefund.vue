        ...
        >
          <template #bodyCell="{ column, text, record }">
            ...
            <template v-if="column.dataIndex === 'operation'">
              <AButton
                v-auth="'/v1/finance/advances/standing'"
                type="link"
                size="small"
              ...
              <template v-if="record.status == '1'">
                <a-divider v-auth="'/v1/finance/advances/standing/confirm'" type="vertical" />
                <AButton
...
<script lang="ts" setup>
  ...
  const goOrderDetails = (record) => {
    const flag = hasPermission('/v1/order/refund/detail')
    if (!flag) {
  ...
  const goDetail = (record) => {
    router.push({
      path: '/financial/account/PaymentRefundDetail',
      query: { id: record.id },
    })
  }
