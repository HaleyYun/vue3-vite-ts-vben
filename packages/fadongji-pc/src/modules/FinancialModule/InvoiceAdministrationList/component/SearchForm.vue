...
<script setup lang="ts">
  ...
  const [registerDefaultConfig, { getFieldsValue }] = createFormRegisterInstance({
    ...
    fieldMapToTime: [['searchTime', ['startApplyTime', 'endApplyTime'], 'YYYY-MM-DD']], // 用于将表单内时间区域的应设成 2 个字段
  ...
  const handlerSubmit = (values) => {
    let data = values
    if (data.startApplyTime) {
      data.startApplyTime = data.startApplyTime + ' 00:00:00'
      data.endApplyTime = data.endApplyTime + ' 23:59:59'
    }
