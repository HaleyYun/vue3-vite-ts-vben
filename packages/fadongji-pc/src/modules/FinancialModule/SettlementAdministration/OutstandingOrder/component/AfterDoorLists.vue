    ...
    >
      <template #bodyCell="{ column, text, record }">
        ...
        <template v-if="column.dataIndex == 'door_time'">
          <div>{{ tabData.homeStartTime }}<br />{{ tabData.homeEndTime }}</div>
        </template>
        <!--修理厂-->
        ...
        <template v-if="column.dataIndex == 'set_master'">
          <div>{{ tabData.masterName }}</div>
        </template>
        <!--地区-->
        <template v-if="column.dataIndex == 'address'">
          <div>{{ tabData.orderAddress && tabData.orderAddress.addressDetail }}</div>
        </template>
        <!--订单状态-->
        <template v-if="column.dataIndex == 'order_statusStr'">
          <div :class="tabData.statusName === '待接单' ? 'text-red-500' : ''">{{
            tabData.statusName
          }}</div>
        </template>
