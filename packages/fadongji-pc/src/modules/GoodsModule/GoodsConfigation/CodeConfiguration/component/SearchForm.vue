<template>
  ...
  <script setup lang="ts">
    ...
    const [registerDefaultConfig, { getFieldsValue }] = createFormRegisterInstance({
      ...
      fieldMapToTime: [['searchTime', ['activatedBeginTime', 'activatedEndTime'], 'YYYY-MM-DD']], // 用于将表单内时间区域的应设成 2 个字段
    ...
    const handlerSubmit = (values) => {
      let data = values
      if (data.activatedBeginTime) {
        data.activatedBeginTime = data.activatedBeginTime + ' 00:00:00'
        data.activatedEndTime = data.activatedEndTime + ' 23:59:59'
      }
