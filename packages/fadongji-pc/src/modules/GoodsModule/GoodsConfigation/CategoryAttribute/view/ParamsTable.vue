      ...
      >
        <a-col :span="8">
          <a-form-item name="name" label="属性名称">
            <a-input
              style="width: 100%"
              v-model:value="formSearchState.name"
              placeholder="输入名称"
            />
          </a-form-item>
        </a-col>
        <a-col :span="8">
          <a-form-item name="id" label="商品类别">
            <a-cascader
  ...
  <div class="p-24px bg-white">
    <EraTableTitle :total="total">
      <template #tableEdit>
        <a-space>
          <a-button
            v-auth="'/v1/goods/param/property'"
          ...
          <a-button
            v-auth="'/v1/goods/param/property/save'"
            type="primary"
            @click.stop="addNewHandler"
          >
            <PlusOutlined />
            添加属性
          </a-button>
    ...
    >
      <template #bodyCell="{ column, record }">
        ...
        <template v-if="column.key === 'isRequired' && record.hierarchy === 3">
          <div>
            <a-switch
              checked-children="是"
              un-checked-children="否"
              v-model:checked="record.isRequired"
              @change="isRequiredChange($event, record)"
            />
          </div>
        </template>
        <template v-if="column.key === 'action'">
          <div v-if="record.hierarchy === 2">
            <a v-auth="'/v1/goods/param/property/save'" @click="addValueByCategory(record)"
              >添加属性</a
            >
          </div>
          <div v-if="record.hierarchy === 3">
            <a v-auth="'/v1/goods/param/property/update'" @click="editSkuHandler(record)">编辑</a>
            <a-divider v-if="!record.isInUse" type="vertical" />
            <a-popconfirm
              v-if="!record.isInUse"
              v-auth="'/v1/goods/param/property'"
              title="确认要删除吗？"
      ...
      <div class="p-3">
        ...
        >
          ...
          <a-form-item
            label="录入方式"
            name="entryMethod"
            :rules="[{ required: true, message: '请选择录入方式', trigger: 'blur' }]"
          >
            <a-radio-group
              :disabled="editabled"
              v-model:value="postForm.entryMethod"
              name="radioGroup"
            >
              <a-radio :value="1">从列表选择</a-radio>
              <a-radio :value="2">手动录入</a-radio>
            </a-radio-group>
          </a-form-item>
          <a-form-item
            v-if="postForm.entryMethod === 1"
            label="属性值类型"
            name="type"
            :rules="[{ required: true, message: '请选择属性值类型', trigger: 'blur' }]"
          >
            <a-radio-group v-model:value="postForm.type" name="radioGroup">
          ...
          <a-form-item
            v-if="postForm.type === 2 && postForm.entryMethod === 1"
            label="属性单位"
            name="unit"
                  ...
                  <div class="flex items-center">
                    ...
                    <div class="pl-10px w-165px">
                      <a-space>
                        ...
                        <a href="javascript:void(0)" @click="removePostFormValue(index)">删除</a>
                      </a-space>
                    </div>
                  </div>
                </div>
          ...
          >
            <a-switch v-model:checked="postForm.isRequired" />
          </a-form-item>
          <a-form-item
            label="是否启用"
            name="isEnable"
...
<script lang="ts" setup>
  ...
  const defaultPostForm = {
    ...
    isEnable: true,
    isRequired: true,
    entryMethod: 1, //录入方式 1从列表选择  2手动录入
    name: '',
    type: 1, // 1 字符  2 数字
    unit: '',
    value: '',
  ...
  const batchSelectedRowKeys = ref([])

  const hasSelected = computed(() => batchSelectedRowKeys.value.length > 0)
  ...
  const sortEvent = async (param) => {
    const { code } = await AdminHttp.goodsParamPropertySort(param)
    if (code == '20001') {
      ...
      async onOk() {
        try {
          return await new Promise((resolve, reject) => {
            AdminHttp.goodsParamPropertyDeleteBatch({
  ...
  const isEnableChange = async (val, record) => {
    const { code } = await AdminHttp.goodsParamPropertyForbidden({
      id: record.id,
      value: val,
    })
    if (+code === 20001) {
  ...
  const isRequiredChange = async (val, record) => {
    const { code } = await AdminHttp.goodsParamPropertyRequired({
      id: record.id,
      value: val,
    })
    if (+code === 20001) {
  ...
  const deleteAtrributeItem = async (record) => {
    try {
      const ret: any = await AdminHttp.goodsParamProperty$id({ id: record.id })
      if (+ret.code === 20001) {
        message.success('删除成功')
        const cData = expandedDataStorage.value.find((item) => item.id === record.categoryId)
        if (cData) {
  ...
  const initCategoryData = async () => {
    try {
      ...
      const { data } = await AdminHttp.goodsParamPropertyQueryCategory({
        ...queryForm,
        ...pageQuery,
      })
  ...
  const addValueHandler = () => {
    const flag = postFormValue.value.every((item) => item.value)
    if (!flag) {
      return message.warn('请完善属性值后操作')
    }
    const valueForm: any = {
      value: '',
      key: new Date().getTime(),
  ...
  const expandHandler = async (expanded, record) => {
    if (record.hierarchy === 2) {
      ...
      // 加载数据
      const { data } = await AdminHttp.goodsParamPropertyQueryList({
        categoryId: record.id,
  ...
  const onFinishForm = async () => {
    ...
    try {
      if (editabled.value) {
        ...
        try {
          loading.value = true
          const ret = await AdminHttp.goodsParamPropertyUpdate(postForm.value)
          loading.value = false
          if (+ret.code === 20001) {
        ...
        try {
          // 新增
          loading.value = true
          const ret = await AdminHttp.goodsParamPropertySave(postForm.value)
          loading.value = false
          if (+ret.code === 20001) {
