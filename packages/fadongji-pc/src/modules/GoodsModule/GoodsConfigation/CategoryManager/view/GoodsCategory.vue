  ...
  <div>
    ...
    <div class="table-list px-24px">
      <div class="form-list-choice pt-24px flex items-center">
        <div>
          <a-space>
            <a-button type="danger" v-auth="'/v1/goods/category/batch/delete'" :disabled="!hasSelect" @click="batchDeleteHandler"
              >批量删除
            </a-button>
            <a-button type="primary" v-auth="'/v1/goods/category/save'" @click="addNewHandler">添加新分类</a-button>
          </a-space>
        </div>
    ...
    >
      <template #bodyCell="{ column, record }">
        ...
        <template v-if="column.dataIndex === 'addChildCategory' && record.hierarchy === 1">
          <a v-auth="'/v1/goods/category/save'" @click="addChildCategory(record)">添加子分类</a>
        </template>
    ...
    <div class="p-3">
      ...
      >
        <a-form-item
          ...
          label="一级分类"
          name="parentId"
          :rules="[{ required: true, message: '请选择一级分类', trigger: 'blur,change' }]"
        >
        ...
        <a-form-item
          ...
          label="二级分类"
          name="name"
          :rules="[{ required: true, message: '未填写二级分类', trigger: 'blur' }]"
        >
          <a-input
...
<script lang="ts" setup>
      ...
      async onOk() {
        try {
          return await new Promise((resolve, reject) => {
            AdminHttp.goodsCategoryBatchDelete({
  ...
  const deleteItem = async (record) => {
    const { code } = await AdminHttp.goodsCategoryBatchDelete({ ids: [record.id] })
    if (+code === 20001) {
  ...
  const getDisabled = (record) => {
    if (+record.hierarchy === 2) {
      const pobj: any = tableData.value.find((item) => item.id === record.parentId)
  ...
  const changeIsEnableEvent = async (val, record) => {
    const { code } = await AdminHttp.goodsCategoryDisableTypeUpdate({
      id: record.id,
  ...
  const modifyEvent = (record) => {
    ...
    if (record.hierarchy === 2) {
      options.value = tableData.value.find((item) => item.id === record.parentId)
    }
  ...
  const onFinishForm = async () => {
    if (editabled.value) {
      ...
      loading.value = true
      try {
        const { code } = await AdminHttp.goodsCategoryUpdate({
          ...postForm.value,
      ...
      loading.value = true
      try {
        const { code } = await AdminHttp.goodsCategorySave(postForm.value)
        loading.value = false
        if (+code === 20001) {
  ...
  const expandHandler = async (expanded, record) => {
    ...
    if (expanded) {
      // 展开
      // 获取二级数据 并添加到一级节点里面
      const { data } = await AdminHttp.goodsCategorySecondCategory({
        id: record.id,
      })
      tableData.value = tableData.value.map((item) => {
  ...
  const rowSelection = ref({
    ...
    getCheckboxProps: (record) => ({
      disabled: !record.isEnable, // Column configuration not to be checked
    }),
  })
  ...
  const initGoodsCategoryData = async () => {
    try {
      const { data } = await AdminHttp.goodsCategoryList({
        ...paginations,
        orderProperty: 'sort',
        orderType: 'ASC',
      })
