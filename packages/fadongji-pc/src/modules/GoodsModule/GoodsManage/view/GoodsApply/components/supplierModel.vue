<template>
    ...
    >
      <div v-if="profitColumn.length" class="dialog-inner pt-20px px-20px">
        <div class="template--title">商品分润</div>
  ...
  <script lang="ts" setup>
    ...
    const confirmSave = async () => {
      ...
      if (!flag) {
        return message.warn('商品分润不可为空或0')
      }
      if (total > +props.cashPrice) {
        return message.warn('商品分润不能大于商品总价')
      }
      const { code } = await adminHttp.goodsGoodsProfitSet(postForm)
      if (+code === 20001) {
        handleClose()
