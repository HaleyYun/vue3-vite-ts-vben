      ...
      >
        <template #bodyCell="{ column, text, record }">
          ...
          <template v-if="column.dataIndex === 'platformFee'">
            <a v-if="transferFee('platform', record.shareProfitInfo)">
          ...
          <template v-if="column.dataIndex === 'recommendFee'">
            <a v-if="transferFee('recommender', record.shareProfitInfo)">
...
<script setup name="goodsServiceProviderList" lang="ts">
  ...
  const getGoodsGoodsListCount = async () => {
    try {
      const { code, data } = await adminHttp.GOODS.goodsGoodsAgentListCount()
      if (code === '20001') {
  ...
  const loadTableData = async () => {
    ...
    console.log(tableData.value, 'asdjkfpioajkfpioadsjkfpakjp')
  }
  const initData = () => {
    // getGoodsGoodsListCount()
    loadTableData()
