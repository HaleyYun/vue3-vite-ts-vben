<template>
  ...
  <div class="attribute-search__btn">
    ...
    <AButton @click="addAttBute()">添加属性</AButton>
  </div>
  <!-- table 区域 -->
          ...
          <div>
            <div v-for="(c, i) in childrenData[r.id]" :key="i">
              <div class="all-built">
                ...
                <div class="add-atrribute" @click="addRowAttribute(item)">添加属性</div>
              </div>
              <Table
                v-if="c.open"
  ...
  >
    <template #content="{ row }">
      <template v-if="VoUtils.hasOwn(row, 'isSelect')"></template>
      <template v-if="VoUtils.hasOwn(row, 'Radio')">
        <RadioGroup v-model:value="attributeValue" @change="changeAttributeFormat" disabled>
  ...
  >
    <div class="p-10">
      ...
      >
        ...
        <AFormItem label="属性录入方式" name="twoGroup">
          <a-radio-group v-model:value="formState['twoGroup']">
        ...
        <template v-if="appear">
          <AFormItem name="company" label="单位">
            <AInput v-model:value="formState.company" style="width: 50px" />
          </AFormItem>
        </template>
        ...
        <AFormItem name="switch" label="是否启用">
          <ASwitch v-model:checked="formState.switch" />
        </AFormItem>
        <AFormItem name="required" label="是否启用">
          <ASwitch v-model:checked="formState.required" />
        </AFormItem>
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const nickHandler = (id, cid, flag = true) => {
        ///v1/goods/sku/property/query/list /v1/goods/param/property/query/list
        AdminHttp.goodsParamPropertyQueryList({ categoryId: cid })
          .then((res) => {
      ...
      const deleteAtrributeItem = (item) => {
        ...
        AdminHttp.goodsParamProperty$id({ id: item.id })
          .then(() => {
      ...
      //添加属性//v1/goods/param/property/save
      const BrandsAdd = () => {
        formRef.value
          .validate()
          .then((res) => {
            ...
            createMessage.loading({ content: 'Loading...', key: loadingCtxkey })
            AdminHttp.goodsParamPropertySave({
              categoryId: formState.brandName,
              isEnable: formState.switch,
              name: formState.brandLoGo,
              type: formState.radioGroup,
              isRequired: formState.required,
              entryMethod: formState.twoGroup,
              unit: formState.company,
      ...
      // /v1/goods/param/property/query/category
      const loadTableData = async () => {
        try {
          ...
          console.log(`获属性11列表失败:${err.message}`)
        } finally {
        }
      }
      loadTableData()
