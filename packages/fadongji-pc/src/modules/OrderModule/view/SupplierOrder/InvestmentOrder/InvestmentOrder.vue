                ...
                <div class="absolute flex items-center left-0px right-0px top-16px z-200 h-56px bg-gray-100">
                  <div class="flex bg-gray-100 px-20" v-if="record.status === 1210">
                    ...
                    <div class="flex ml-50px">
                      <div class="bg-gray-100">下单时间：</div>
                      <div>{{ record.createTime }}</div>
                    </div>
                    <div class="flex ml-50px">
                      <div class="bg-gray-100">订单金额：</div>
                      <div>{{ record.goodsAmount }}</div>
                    </div>
                    <div class="flex ml-50px">
                      <div class="bg-gray-100">订单状态：</div>
                  ...
                  >
                    ...
                    >
                      <div v-if="item.periodDays != 0">
                        <div
                          >{{ item.periodDays }}天账期{{ item.originalPrice }}元<span
...
<script setup name="investmentOrder" lang="ts">
  import { reactive, ref, watch } from 'vue'
  import ReminTime from '/@/modules/components/RemainTime.vue'
  import InvestFilterForm from '/@/modules/OrderModule/view/SupplierOrder/InvestmentOrder/component/InvestFilterForm.vue'
  ...
  import { containerConfig } from '/@/modules/OrderModule/view/SupplierOrder/InvestmentOrder/data/InvestContainerData'
  ...
  const goDetails = (record) => {
    console.log(record.id, '子订单record')
    go.push({
  ...
  const handlerStatus = async () => {
    const { data } = await adminHttp.orderInvestStatusCount({ shopPlatformCode: 'supplier' })
    tabsList.value = [
      {
        key: 'one',
        name: '全部订单',
  ...
  const investEnumStatusEvent = async () => {
    const res = await adminHttp.Order.investEnumStatus({ shopPlatformCode: 'supplier' })
    if (res.code == '20001') {
