                      ...
                      <tbody>
                        <div class="body_top_con">
                          <div class="top_con_shop">
                            <span class="top_con_num my-16.5px">订单号：{{ item.id }}</span>
                           <span class="top_con_time my-16.5px">下单时间：{{ item.createTime }}</span>
                            <span v-if="item.isValet" class="order-tag ml-16px">代下单</span>
                        ...
                        <tr class="body_content">
                          ...
                          >
                            <div>
                              <a
                                v-if="item.isWarn"
                                class="border-solid border-1 border-red-600 text-red-600"
                                >催</a
                              >{{ item.statusName }}
                            </div>
                          </td>
                          ...
                          >
                            <div class="flex" style="flex-direction: column">
                              <div v-if="item.statusName === '待付款'" class="ant-system__red">
                                <ReminTime :nextStageTime="item.nextStageTime"></ReminTime>
                              </div>
                              <div>
                                <AButton
...
<script setup lang="ts">
  ...
  import AgentSaleFilterForm from '/@/modules/OrderModule/view/AgentOrder/AgentSaleOrder/component/AgentSaleFilterForm.vue'
  import { clearObjectUnalbeAttr } from '/@/utils/helper/formHelper'
  import { containerConfig } from '/@/modules/OrderModule/view/AgentOrder/AgentSaleOrder/data/AgentSaleContainerData'
  ...
  const loadTableData = async () => {
    ...
    const { records, total } = await adminHttp
      ...
      .catch((err) => {
        // console.log(err)
        console.log(`获取服务商订单列表失败:${err.message}`)
  ...
  const handlerStatus = async () => {
    const { data } = await adminHttp.orderSellStatusCount({ shopPlatformCode: 'agent' })
    tabsList.value = [
      {
        key: 'one',
        name: '全部订单',
