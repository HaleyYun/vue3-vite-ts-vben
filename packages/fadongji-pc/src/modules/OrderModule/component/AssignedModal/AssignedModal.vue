...
<script lang="ts" setup>
  ...
  import { assignColumns } from './data'
  ...
  const assignOrder = () => {
    if (new Date(props.orderInfo.homeStartTime).getTime() > new Date().getTime()) {
      ...
      let date = VoUtils.timeFormat(new Date(), 'yyyy-mm-dd')
      let hours = new Date().getHours()
      let timeOptions = timeSelectOptions.find((item) => {
        let optionHours = item.value.split(':')
        return optionHours[0] > hours
      })
      if (timeOptions) {
        let start = date + ' ' + timeOptions.value.split('~')[0]
        let endTime = date + ' ' + timeOptions.value.split('~')[1]
        console.log(endTime)
        filterData.value = {
          homeStartTime: start,
          homeEndTime: endTime,
        ...
        nextTick(() => {
          DoorAssign.value && DoorAssign.value.setFormValue(start, endTime)
        })
      } else {
        // 当前时间已经超时了，取第二天第一个时间节点
        let nextDate = VoUtils.timeFormat(new Date().getTime() + 86400000, 'yyyy-mm-dd')
        let start = nextDate + ' ' + timeSelectOptions[0]['value'].split('~')[0]
        let endTime = nextDate + ' ' + timeSelectOptions[0]['value'].split('~')[1]
        console.log(endTime)
        filterData.value = {
          homeStartTime: start,
          homeEndTime: endTime,
  ...
  const handlerSubmit = (values) => {
    ...
    if (!values.homeStartTime) {
      createMessage.error('请选择指派时间！')
      return
    }
    filterData.value = {
      ...values,
