
<script>
  // #ifdef APP-PLUS

  const vocenJpush = uni.requireNativePlugin('vocen-jpush')
  const vocenJpushGlobalEvent = uni.requireNativePlugin('globalEvent')
  // #endif
  import IM from '@/common/Im/Im'
  import { linkToEasy } from '@/common/helper'
  import { VoHttp } from "@/common/unifyGlobalReg";
  import {iosEnvVersion,androidEnvVersion} from "@/common/envVersion";
  export default {
    onLaunch(e) {



		// #ifdef APP-PLUS
					plus.screen.lockOrientation("portrait-primary")

		// #endif

      // 1.1.0版本之前关于http拦截器代码，已平滑移动到/common/http.interceptor.js中
      // 注意，需要在/main.js中实例化Vue之后引入如下(详见文档说明)：
      // import httpInterceptor from '@/common/http.interceptor.js'
      // Vue.use(httpInterceptor, app)
      // process.env.VUE_APP_PLATFORM 为通过js判断平台名称的方法，结果分别如下：
      /**
       * h5，app-plus(nvue下也为app-plus)，mp-weixin，mp-alipay......
       */
      // #ifdef H5
      (function(w, d, s, q, i) {
        w[q] = w[q] || [];
        var f = d.getElementsByTagName(s)[0],j = d.createElement(s);
        j.async = true;
        j.id = 'beacon-aplus';
        j.src = 'https://d.alicdn.com/alilog/mlog/aplus/' + i + '.js';
        f.parentNode.insertBefore(j, f);
      })(window, document, 'script', 'aplus_queue', '203467608');

      //集成应用的appKey
      aplus_queue.push({
        action: 'aplus.setMetaInfo',
        arguments: ['appKey', '634928ac05844627b565dd7a']
      });
      /************************以下内容为可选配置内容****************************/
      //sdk提供手动pv发送机制，启用手动pv(即关闭自动pv)，需设置aplus-waiting=MAN;
      //注意：由于单页面路由改变时不会刷新页面，无法自动发送pv，所以对于单页应用，强烈建议您关闭自动PV, 手动控制PV事件
      aplus_queue.push({
        action: 'aplus.setMetaInfo',
        arguments: ['aplus-waiting', 'MAN']
      });
      //是否开启调试模式
      aplus_queue.push({
        action: 'aplus.setMetaInfo',
        arguments: ['DEBUG', true]
      });
      //是否指定用作计算umid的id类型，默认为cnaid，目前支持:
      //1. 微信和QQ: openid; 字节和百度 anonymousid; 支付宝 alipay_id
      //2. 微信、QQ、字节、百度平台的 unionid
      //3. 业务方自己生成的随机id uuid
      aplus_queue.push({
        action: 'aplus.setMetaInfo',
        arguments: ['aplus-idtype', 'xxxx'] //取值参考见附表1
      });

      //按钮发就可以了
      // const {aplus_queue} = window;
      // aplus_queue.push({
      //   action: 'aplus.sendPV',
      //   arguments: [{
      //         is_auto: false
      //       }, {
      //         param1:111,
      //     param2: '222'
      //   }]
      //   });
      // #endif
      this.voJPushReviceDataCallbackFn()
      this.imConnectFn()
    },
    onError(err) {
      console.log(err)
    },
    onHide() {
      this.closeInnerAudioContext()
    },
    onUnload() {
      this.closeInnerAudioContext()
    },
    methods: {
      imConnectFn() {
        // im连接
        const userInfo = uni.getStorageSync('userInfo')
        if (userInfo && userInfo.access_token) {
          let imCtx = null
          imCtx = new IM()
          this.$store.commit('im/SET_IMCTX', imCtx)
          imCtx.start()
          imCtx.openListen()
        }
      },
      voJPushReviceDataCallbackFn() {
        // #ifdef APP-PLUS
        vocenJpush.voJPushReviceDataCallback({}, (res) => {
          console.log(res)
        })
        vocenJpushGlobalEvent.addEventListener('voJpushEvent', function (message) {
          console.log(message, 'message')

          const result = message.JPushReceiver.result
          if (Number(result.type) === 1) {
            linkToEasy(result.url)
          }
        })
        // #endif
      },
    },
  }
</script>

<style lang="scss">
  /* #ifndef APP-NVUE */
  @import 'assets/demo.scss';
  /* #endif */

  @import '~@vocen/uview-ui/index.scss';

  .theme-red {
    --color-primary: #e50012;
    --color-clicked: #4e536f;
    --color-disabled: #f6f7f8;
    --color-text-disabled: #f6f7f8;
    --color-shadow: rgba(34, 40, 75, 0.15);
  }

  .theme-blue {
    --color-primary: #ff5319;
    --color-clicked: #ec404d;
    --color-disabled: #fdf2f3;
    --color-text-disabled: #faccd0;
    --color-shadow: rgba(34, 40, 75, 0.15);
  }

  ::-webkit-scrollbar {
    display: none;
  }
</style>
