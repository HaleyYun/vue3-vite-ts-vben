  ...
  <view class="serve">
    ...
    <block v-if="imgList && imgList.length">
      ...
      <view class="serve-video">
        <view v-for="(info, index) of imgList" :key="index + '1'" class="serve-video__outside">
          <view v-if="info.type === 2" @click="choseVideo(info)">
            <image :src="info.src + '&type=3'" class="outside-size" @error="imgErr" />
          ...
          <image
            ...
            :src="info.src"
            class="outside-size"
            @click="previewFn(index, [info.src])"
            @error="imgErr"
          />
        </view>
      </view>
...
<script>
  export default {
    ...
    methods: {
      ...
      // 重组图片预览列表
      filterImgList(info) {
        if (info.installEvaluate && info.installEvaluate.content) {
          this.imgList = info.installEvaluate.content.pic
        }
      },
    },
  }
