    ...
    >
      ...
      <view class="setting-content flex1 overflow-y">
        ...
        <EraCell title="修改手机号" @click.native="changeMobileClick">
          <view slot="value">
            <view class="setting-content__right"> {{ encryptPhone(userInfo.userName) }}</view>
          </view>
          <block slot="value">
            <VoIcon :opacity="0.45" :size="20" color="#000000" name="arrow-right" />
          </block>
        ...
        <EraCell :border="false" title="注销集师傅" url="/pagesSupplier/Setting/LogoutSupplier">
          <block slot="value">
            <VoIcon :opacity="0.45" :size="20" color="#000000" name="arrow-right" />
          </block>
        </EraCell>
  ...
  export default {
    ...
    onLoad() {
      ...
      uni.getStorage({
        key: 'switchSelecte',
        success: (res) => {
          this.switchSelecte = res.data
        },
      })
      this.getPersonalized()
    },
    onShow() {
      console.log(storage.get('switchSelecte'))
    },
    methods: {
      versionFn() {
      ...
      pushWebClick(e) {
        ...
        if (e === '1') {
          ...
          console.log(111111)
          // #ifdef H5
          // this.$linkToEasy('/pages/CommonWeb/UserPolicy')
        ...
        if (e === '2') {
          title = '隐私政策'
          // #ifdef H5
          // this.$linkToEasy('/pages/CommonWeb/PrivacyPolicy')
