<template>
  <view class="block">
    ...
    <view class="block-list m-t-16">
      <view class="flex flex-justify-between">
        <view>发货方式：</view>
        <view class="block-list__right">{{
          returnInfoList.deliveryType === 1 ? '物流发货' : '无需物流'
        }}</view>
      </view>
    </view>
    ...
    <view v-show="returnInfoList.deliveryType === 1" class="block-list">
      <view class="flex flex-justify-between">
        <view>运单号：</view>
        <view class="block-list__right">{{ returnInfoList.waybillNum }}</view>
      </view>
      <view class="flex flex-justify-between m-t-16">
        <view>物流公司：</view>
    ...
    <view v-if="returnInfoList.confirm" class="block-confirm">
...
<script>
  export default {
    ...
    created() {
      console.log('收到的数据', this.returnInfoList)
    },
    methods: {
      confirmReceipt() {
      ...
      lookPreviewImage(index) {
        let imageList = []
        this.returnInfoList.imageList.forEach((item) => {
          imageList.push(item.proofImg)
        })
        // 预览图片
        uni.previewImage({
          urls: imageList,
