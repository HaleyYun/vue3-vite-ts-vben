<template>
  <view class="info">
    ...
    <view v-else class="info-name overTwoEllipsis m-b-24">
      <text v-show="recordList.categoryType" class="good-tips m-r-8">{{
        recordList.categoryType
      }}</text>
      {{ recordList.goodsName }}
    ...
    <!--    <view v-if="recordList.goodsVin" class="info-item fz-28 color-gray">-->
    <!--      <view class="info-item__no">{{ showContent ? recordList.goodsVin : content }}</view>-->
    <!--      <view-->
    <!--        v-if="showOpen"-->
    <!--        :class="{ changeBottom: showContent }"-->
    <!--        class="flex flex-vertical-c info-item__fixed"-->
    <!--        @click="showContent = !showContent"-->
    <!--      >-->
    <!--        <view>{{ showContent ? '收起' : '展开' }}</view>-->
    <!--        <VoIcon v-if="!showContent" :size="24" color="#d9d9d9" name="arrow-bottom-line" />-->
    <!--        <VoIcon v-else :size="24" color="#d9d9d9" name="arrow-top-line" />-->
    <!--      </view>-->
    <!--    </view>-->

    <view class="info-con">
      <view class="m-b-16">入库仓库：{{ recordList.inWarehouseName }}</view>
      <view class="m-b-16 info-con__time">商品数量：{{ recordList.changeNumber }}</view>
      <!--      <view class="flex flex-wrap">-->
      <!--        <view class="m-b-16">入库仓库：{{ recordList.inWarehouseName }}</view>-->
      <!--        <view class="m-b-16 info-con__time algin-top info-con__list">商品数量：{{ recordList.changeNumber }}</view>-->
      <!--      </view>-->
      <view class="info-con__list m-r-60">出货仓库：{{ recordList.outWarehouseName }}</view>
...
<script>
  export default {
    ...
    created() {
      ...
      // this.content = this.recordList.goodsVin ? this.recordList.goodsVin.slice(0, 100) : ''
