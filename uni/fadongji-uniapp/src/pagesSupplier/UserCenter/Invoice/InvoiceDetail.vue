<template>
  ...
  >
    ...
    <view class="flex1">
      <view v-for="(item, index) in list" :key="index" class="invoiceDetail-item">
      ...
      >
        <view>1张发票，包含{{ detailData.orderCount || 0 }}个订单</view>
    ...
    <!--    <view class="invoiceDetail-bottom p-b-safe-area">-->
    <!--      <EraButton class="m-t-20" text="下载全部发票" @click="downloadInvoice" />-->
    <!--    </view>-->
  </view>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
       ...
       */
      orderInfoClick() {
        this.$linkToEasy(`/pagesSupplier/UserCenter/Invoice/InvoiceOrderInfo?id=${this.id}`)
      },
      /**
       * 开票记录详情
       */
      ...
      async downloadInvoice() {
        ...
        for (let i = 0; i < this.picUrlList.length; i++) {
          ...
          try {
            ...
            // #endif
            // #ifdef H5
            await downloadImage(this.picUrlList[i], '安装指导')
            // #endif
          } catch (e) {
            console.log(e);
          }
