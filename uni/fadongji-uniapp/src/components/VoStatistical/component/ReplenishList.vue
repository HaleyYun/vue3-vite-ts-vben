    ...
    methods: {
      async getData() {
        try {
          ...
          this.loading = true
          let params = {
            investType: 1, // 投资类型（1：本地投资，2：全国投资 本地进货 全国批发
      ...
      async purchaseWholesale() {
        try {
          const { code, data } = await this.$VoHttp.apiOrderStatisticsInvestTotal({
            ...
            endTime: this.timeParams.selectTime[1],
            timeType: this.timeParams.timeType == 4 ? 1 : this.timeParams.timeType, // 时间类型,1:天,2:周,3:月
            investType: 1, //进货是1 批发是2
          })
          if (code == '20001') {
            this.totalList = [
              { label: '进货金额（元）', value: data.purchaseNumTotal },
              { label: '进货量', value: data.salesNumTotal },
            ]
          }
        } catch (e) {
          console.log(e)
