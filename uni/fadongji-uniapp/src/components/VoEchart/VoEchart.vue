<template>
  <view class="vo-echart" :style="customStyle" v-if="canvasId" ref="limeEchart">
    <!-- #ifndef APP-NVUE -->
    <canvas
      class="vo-echart__canvas"
      v-if="use2dCanvas"
      type="2d"
      :id="canvasId"
      :style="canvasStyle"
    ...
    <web-view
      ...
      ref="webview"
      src="/uni_modules/vo-echart/static/index.html"
...
<script>
  ...
  export default {
    ...
    computed: {
      canvasId() {
        return `vo-echart${this._uid || this._.uid}`
      },
      offscreenCanvasId() {
    ...
    created() {
      ...
      this.use2dCanvas =
        ...
        !((/ios/i.test(platform) && /7.0.20/.test(version)) || /wxwork/i.test(environment)) &&
        !this.isPC
      // #endif
      // #ifdef MP-TOUTIAO
    ...
    methods: {
      ...
      setChart(callback) {
        ...
        if (typeof callback === 'function') {
          this.$refs.webview.evalJs(
            `setChart(${JSON.stringify(callback.toString())}, ${JSON.stringify(this.roptions)})`,
          )
        }
        // #endif
      },
      ...
      async init(echarts, callback) {
        ...
        if (find) {
          ...
          callback(this.chart)
          this.$refs.webview.evalJs(
            `init(${JSON.stringify(find.toString())}, ${JSON.stringify(this.roptions)})`,
          )
        } else {
          console.error('callback 非 function')
        }
