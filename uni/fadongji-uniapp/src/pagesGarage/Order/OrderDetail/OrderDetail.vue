  ...
  <view>
    <view class="detail flex flex-column">
        ...
        <view class="detail-address m-t-16">
          ...
          >
            <view class="detail-address__list flex" @click="goodDetail(item)">
              <image v-if="item.pic[0].indexOf('mp4')!=-1||item.pic[0].indexOf('MP4')!=-1" :src="item.pic[0]+'&type=3'" class="list-image" />
      ...
      <view v-else-if="type === 1120" class="detail-bottom flex flex-justify-r">
        <view class="p-b-safe-area" />
        <view v-show="orderData.payPrice > 0" class="detail-bottom__button" @click="bottomRefundClick(false)">申请退换货</view>
      ...
      <view v-else-if="type === 1130" class="detail-bottom flex flex-justify-r">
        <view class="p-b-safe-area" />
        <!--控制退换货按钮显示-->
        <view v-show="orderData.payPrice > 0" class="detail-bottom__button" @click="bottomRefundClick(true)">申请退换货</view>
...
<script>
  ...
  import { appSource, compareTime, goWechat, hideLoading, showLoading } from '@/common/helper'
  ...
  export default {
    name: 'OrderDetail',
    components: { CodeService, CancelOrder, RefundActionPopup, AddServicePrice },
    data() {
      return {
        totalPrice: 0,
        showPay: false,
    ...
    async onShow() {
      ...
      if (this.orderData.status === 1130 || this.orderData.status === 1140) {
    ...
    methods: {
      ...
      async payHandler(info) {
        ...
        if (info.payTypeCode === 1) {
          ...
          goWechat(
            `/pages/pay/pay?req=${JSON.stringify(req)}`,
          )
          return
        }
        ...
        this.$VoHttp
          ...
          .then((res) => {
            ...
            if (res.code === '20001') {
              ...
              setTimeout(() => {
                this.$linkToEasy('/pagesGarage/Order/ReturnExchange/RefundHistory')
                this.getOrderDetailRequest()
              }, 1500)
            } else {
