    ...
    <view class="install-box">
      ...
      <view v-if="video.length">
        ...
        <view class="relative" @click="playVideo()">
          <image :src="video[0] + '&type=3'" class="install-image" mode="widthFix" />
          <image class="play" src="/static/created/circle/circle_video_play.png" />
        </view>
        <!-- <video id="myVideo" class="install-video" :src="video[0]"></video> -->
      </view>
      <EraPreviewVideo ref="EraPreviewVideo" :poster="video[0] + '&type=3'" :videoSrc="video[0]" />
    </view>
  </view>
</template>
<script>
  ...
  export default {
    ...
    mounted() {
      console.log('this.info.serviceConfiguration', this.info.serviceConfiguration)
      if (this.info.serviceConfiguration && this.info.serviceConfiguration.length) {
        this.info.serviceConfiguration.forEach((item) => {
          if (item.key === '安装指导') {
            this.imgs = JSON.parse(item.value)
          }
          if (item.key === '安装视频') {
            this.video = JSON.parse(item.value)
          }
