<template>
  ...
  >
    <view class="create flex-column">
      <VoNav :title="id ? '修改服务商资料' : '创建服务商'" is-fixed is-have-more>
      ...
      <view v-if="current === 0" class="flex1 overflow-y">
        ...
        >
          <!-- H5注册选择注册类型 -->
          <view v-if="isShare" class="flex flex-row flex-justify-between bg-white p-t-24 p-b-8">
            <view class="fz-32 lh150 color-block">您是</view>
            <view class="flex1">
              <view class="flex flex-row flex-justify-r flex-vertical-c m-b-24">
                <view class="flex flex-row flex-vertical-c">
                  <view
                    class="flex flex-row flex-vertical-c"
                    @click="id ? void 0 : changeReginstType(2)"
                  >
                    <VoIcon v-if="type === 2" :size="24" color="#FF5319" name="select-right" />
                    <VoIcon v-else name="address_select" size="24" />
                    <view class="fz-26 lh150 color-block m-l-8">企业</view>
                  </view>
                  <view
                    class="flex flex-row flex-vertical-c m-l-60"
                    @click="id ? void 0 : changeReginstType(3)"
                  >
                    <VoIcon v-if="type === 3" :size="24" color="#FF5319" name="select-right" />
                    <VoIcon v-else name="address_select" size="24" />
                    <view class="fz-26 lh150 color-block m-l-8">个体工商户</view>
                  </view>
                </view>
              </view>
              <view v-if="type === 2" class="flex flex-row">
                <view class="fz-24 lh150 color-block-45"
                  >企业需要设置对公账户，用于结算及确认收入。</view
                >
              </view>
            </view>
          </view>
          <view class="place-holder m-l-32 m-r-32" />
          <EraFormItem
...
<!--            <VoIcon :opacity="0.25" name="right-arrow" />-->
            <VoIcon :size="20" color="#373A4E" name="address" @click.native.stop="getLocation" />
          </EraFormItem>
          ...
          >
            ...
            <VoIcon
              ...
              color="#000"
              name="close-c"
              @click="formData.detail = ''"
            />
<!--            <VoIcon v-else :size="20" color="#373A4E" name="address" @click="getLocation" />-->
      ...
      <!--  步骤二模块：start  -->
      <view v-if="current === 1" class="flex1 overflow-y">
        ...
        >
          ...
          <EraFormItem
            ...
            required
          >
            <view class="flex flex1 flex-end flex-justify-r">
              <view
      ...
      <!--  步骤二底部固定模块：start  -->
      <view v-else-if="current === 1" class="create-footer flex">
<!--        <EraButton-->
<!--          class="m-r-36"-->
<!--          size="small"-->
<!--          text="上一步"-->
...
<!--        />-->
        ...
        <EraButton
          ...
          @click="nextStep"
        />
        <EraButton v-else disabled size="small" text="下一步" />
      </view>
      <!--  步骤二底部固定模块：end  -->

      <!--  步骤三底部固定模块：start  -->
      <view v-else-if="current === 2" class="create-footer flex">
<!--        <EraButton-->
<!--          class="m-r-36"-->
<!--          size="small"-->
<!--          text="上一步"-->
...
<!--        />-->
        <view class="self-button" @click="lastStep">上一步</view>
        <view v-if="type === 1" class="create-footer__view">
          <EraButton
            v-if="
              formData.legalId &&
        ...
        <view v-else-if="type === 2" class="create-footer__view">
          <EraButton
            v-if="
              formData.legalId &&
              formData.legalStartDate &&
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        ...
        formData: {
          ...
          companyType: 2, //企业类型（2：企业会员, 3：个人会员）
        },
        rules: {
          mobile: [
            {
    ...
    methods: {
      ...
      async checkPhone() {
        ...
        try {
          if (this.isShare) {
            ...
            data = await this.$VoHttp.apiCompanyInvitationH5CheckAgent(param)
          } else {
            data = await this.$VoHttp.apiCompanyInvitationCheckAgent(param)
          }
          if (!data || !data.data) {
            this.isLoading = !this.isLoading
            return
      ...
      /**
       * 切换注册类型
       * @type {Number}  类型：3、个人；2、企业
       */
      changeReginstType(type) {
        if (this.id) return
        this.formData.companyType = this.type = type
      },
      /**
       * 上一步
       */
      ...
      async firstNextStep() {
        ...
        try {
          if (this.isAccept) {
            data = await this.$VoHttp.apiCompanyInvitationH5Agent$POST(params)
          } else {
            data = await this.$VoHttp.apiCompanyInvitationAgent$POST(params)
          }
      ...
      async submitInfo() {
        ...
        try {
          if (this.isAccept) {
            // h5创建
            data = await this.$VoHttp.apiCompanyInvitationH5Agent$PUT(params)
          } else {
            // 原生创建
            data = await this.$VoHttp.apiCompanyInvitationAgent$PUT(params)
          }
          if (!data || !data.data) return
          if (+data.code === 20001) {
            // if (this.id) {
            ...
            this.show = true
            setTimeout(() => {
              // this.$linkToEasy(`./AuthenticationCompleted?state=1&origin=2`)
              this.$linkToEasy(`/pagesChannelManager/ServiceManager/ServiceManager`)
              // this.$backFn()
            }, 1500)
          } else {
      ...
      async initFn() {
        ...
        showLoading()
        try {
          const data = await this.$VoHttp.apiCompanyInvitationAuthInfo({ companyId: this.id })
          if (!data || !data.data) return
          if (+data.code === 20001) {
            console.log(data);
            this.formData = data.data
            this.formData.mobile = data.data.userName
            this.formData.areaStr = data.data.provinceName + data.data.cityName + data.data.areaName
