  ...
  <view class="info flex bg-white">
    ...
    <view class="flex flex-column flex1 m-l-16">
      <view class="flex flex-wrap">
        <view class="info-title">{{ info.fullName || info.storeName || info.userName }}</view>
        <block v-if="info.platformCode === 'agent'" class="m-t-6">
          <view
            v-if="info.checkStatus === 2 && info.isBindMobile"
        ...
        <block v-else>
          <view
            v-if="info.checkStatus === 1 && info.isBindMobile"
            class="flex flex-row flex-vertical-c info-certified orange"
          >
            <VoIcon color="#FF9B05" name="success-fill" size="12" />
            <view class="m-l-6">待审核</view>
          ...
          <view
            v-if="!info.downloadStatus"
            class="flex flex-row flex-vertical-c info-certified red"
          >
            <view class="m-l-6">未下载APP</view>
          </view>
        </block>
      </view>
      <view class="flex p-b-12 m-t-8">
        ...
        <view class="flex-shark-0 flex fz-20 color-block-45 m-l-32">
          <view class="flex flex-column flex-vertical-c" @click="$cellPhone(info.userName)">
            ...
            <view class="m-t-16">电话</view>
          </view>
          <view v-if="+info.checkStatus === 4" class="flex flex-column flex-vertical-c m-l-48">
            <image class="icon-56" src="/static/common/map/chat.png" @click="toChat" />
            <view class="m-t-16">聊天</view>
          </view>
        </view>
      </view>
    </view>
