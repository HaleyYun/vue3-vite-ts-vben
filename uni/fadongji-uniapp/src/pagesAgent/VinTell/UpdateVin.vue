...
export default {
  ...
  methods:{
    sureFn(){
      if(this.type===1){
        // 修改
        this.$linkToEasy(`/pagesAgent/VinTell/VinTell?code=${this.codeInput.join('')}&type=handInput&imageInfoVO=${this.imageInfoVO}`)
      }else{
        // 输入
        if(!this.vinVal){
        ...
        this.$linkToEasy(`/pagesAgent/VinTell/VinTell?code=${this.vinVal}&type=handInput&imageInfoVO=${this.imageInfoVO}`)
    ...
    async vinFn(){
      ...
      if (res && res.data) {
        ...
        if (+data.data.scanResultType === 1) {
          ...
          this.$linkToEasy(
              `/pagesAgent/VinTell/VinTell?code=${data.data.code}&type=${
                  res.type
              }&imageInfoVO=${
