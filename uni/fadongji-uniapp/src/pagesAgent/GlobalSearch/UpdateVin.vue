...
export default {
  ...
  methods:{
    sureFn(){
      if(this.type===1){
        // 修改
        this.$linkToEasy(`/pagesAgent/GlobalSearch/QueryResult?salesArea=${this.salesArea}&code=${this.codeInput.join('')}&companyId=${this.companyId}&userName=${this.userName}&imType=${this.imType}&queryType=${this.queryType}&type=handInput&imageInfoVO=${this.imageInfoVO}`)
      }else{
        // 输入
        if(!this.vinVal){
        ...
        this.$linkToEasy(`/pagesAgent/GlobalSearch/QueryResult?salesArea=${this.salesArea}&code=${this.vinVal}&companyId=${this.companyId}&userName=${this.userName}&imType=${this.imType}&queryType=${this.queryType}&type=handInput&imageInfoVO=${this.imageInfoVO}`)
    ...
    async vinFn(){
      ...
      if (res && res.data) {
        ...
        if (+data.data.scanResultType === 1) {
          ...
          this.$linkToEasy(`/pagesAgent/GlobalSearch/QueryResult?salesArea=${this.salesArea}&code=${data.data.code}&companyId=${this.companyId}&userName=${this.userName}&imType=${this.imType}&queryType=${this.queryType}&type=${res.type}&imageInfoVO=${data.data&&data.data.imageInfoVO&&data.data.imageInfoVO.filePath?data.data.imageInfoVO.filePath:''}`)
          return
        }
      }
