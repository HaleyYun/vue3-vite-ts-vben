    ...
    >
       ...
       <block   v-for="(item, index) in modelCar"
         ...
         >
           ...
           <view class="flex1 color-block-85">
             ...
             <view v-if="item.brandName" class="m-t-4 fz-28 lh150"
      ...
      <block v-else-if="isOverLoading">
        <VoNoData :no-data-tips="['暂无匹配商品']" :paddingTop="0">
                    <block slot="default">
                      <EraButton @click="goConcatFn" size="small" theme-type="Wire" class="m-t-46">联系平台客服</EraButton>
                    </block>
        </VoNoData>
      </block>
    </scroll-view>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      openEdit() {
        this.$linkToEasy(
          '/pagesAgent/GlobalSearch/UpdateVin?imageInfoVO=' +
            this.imageInfoVO +
            '&code=' +
            this.code +
            '&queryType=' +
            ...
            this.companyId +
            '&imType=' +
            this.imType+'&salesArea='+this.salesArea
        )
      },
      /**
       * 确认事件
