<template>
  <view class="cost">
    <VoNav is-have-more title="商品服务费" />
    <view class="flex1 m-t-16 p-b-safe-area cost-content">
      ...
      <view class="m-t-24 fz-24 lh150 color-block-65">
        注：只有商品实际所在的仓库，才会产生仓库服务费。
      </view>
    </view>
  </view>
</template>
  ...
  export default {
    ...
    methods: {
      async initServiceData(id) {
        const { code, data } = await this.$VoHttp.GOODS.apiGoodsGetGoodsServeFee({
          id,
        })
        if (+code === 20001) {
          this.costList = data
