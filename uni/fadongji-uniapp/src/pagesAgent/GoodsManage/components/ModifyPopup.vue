  ...
  <view v-if="info.id" class="modify">
    <u-popup :round="10" :show="show" mode="bottom" @close="close">
      <view class="modify-title flex flex-justify-between">
        ...
        <VoIcon :opacity="0.65" :size="20" color="#000000" name="close" @click="close" />
      </view>
      <EraForm
        ref="form"
        :model="formData"
...
<script>
  ...
  export default {
    ...
    methods: {
      monthInput(value) {
        this.formData.accountPeriodDays = value.replace(/[^\d]/g, '')
      },
      init(info) {
        ...
        if (info.accountPeriodDays && info.accountPrice) {
          this.payType = [1, 2]
        }
      },
      priceAlert() {},
