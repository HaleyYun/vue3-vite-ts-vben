<template>
  <view class="block">
    ...
    <view class="block-info flex">
      <image :src="$transferCosUrl(transactionList.infoImg)" class="block-info__img" />
      <view class="block-info__right flex1">
        ...
        <view class="right-content__bottom flex flex-justify-between">
          <view class="bottom-left flex flex-vertical-c">
            <view class="bottom-left__tips">{{
              transactionList.periodDays === 0 ? '现金价' : transactionList.periodDays + '天账期'
            }}</view>
