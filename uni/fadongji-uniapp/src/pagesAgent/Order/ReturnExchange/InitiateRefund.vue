<template>
  ...
  >
    <VoNav title="退货/退款" />
    <view class="refund-overflow flex1 overflow-y">
      <EraForm
        ref="form"
        :model="formData"
        :rules="rules"
      ...
      >
        ...
        >
          ...
          <VoIcon :opacity="0.45" class="m-l-8" name="right-arrow" />
        </EraFormItem>
      ...
      <view class="refund-area m-t-16">
        <view class="color-block fz-b fz-32 position-relative">
          <span class="red-required">*</span>补充描述和凭证
        </view>
      ...
      <view class="refund-upload">
        <view
          class="refund-upload__name flex flex-justify-between flex-vertical-c position-relative"
        >
          <view class="color-block fz-b fz-32"><span class="red-required">*</span>上传凭证</view>
    ...
    <RefundReasonPopup ref="reasonPopup" :radio-list="typeList" @select="selectType" />
  </view>
</template>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      refundReasonClick() {
        if (this.formData.refundType === '内容' || this.formData.refundType === '请选择退款类型') {
          uni.$u.toast('请选择退款类型')
          return
        }
        ...
        this.$VoHttp
          ...
          .then((res) => {
            if (res.code === '20001') {
              ...
              if (refundInformation.refundPic && refundInformation.refundPic !== '{}') {
                if (refundInformation.refundPic.indexOf(',')) {
                  let list = refundInformation.refundPic.split(',')
                  list.forEach((item) => {
                    this.fileList1.push({ url: item })
                  })
                } else {
                  this.fileList1 = [{ url: refundInformation.refundPic.toString() }]
                }
              }
        ...
        this.$VoHttp
          ...
          .then((res) => {
            ...
            if (res.code === '20001') {
              ...
              setTimeout(() => {
                this.$backFn()
                uni.$emit('back', { index: this.index })
              }, 1500)
            } else {
              uni.$u.toast(res.message || '发起退货失败')
            }
