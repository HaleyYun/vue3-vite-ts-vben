      ...
      <view class="detail flex flex-column">
        ...
        <view class="detail-address m-t-16">
          <view class="detail-address__user flex flex-vertical-c" @click="goImUrl">
            <VoIcon name="smile-m" />
            <view class="flex1 m-l-8 fz-32 fz-b">{{ orderData.shopName }} </view>
          </view>
        ...
        <view class="detail-address m-t-16">
          ...
          <view class="detail-address__profit flex flex-justify-between flex-vertical-c">
            <view class="color-block fz-28">商品名称：</view>
            <view @click="openProfit">
              <view class="flex fz-28 flex-vertical-c">
...
export default {
    ...
    methods: {
       ...
       */
      goImUrl() {
        console.log('this.orderData.buyerId', this.orderData.shopId)
        //根据店铺id获取userId
        getImUserId({ companyId: this.orderData.shopId })
          .then((res) => {
            console.log('res', res)
            if (res.data) {
              const info = {
       ...
       */
      goSellOrdelDetail() {
        this.$linkToEasy('/pagesAgent/Order/OrderDetail/OrderDetail?id=' + this.orderData.orderSellId)
      },
