...
<script>
  import GoodInfoItem from './GoodInfoPromoteItem'
  import CancelOrder from './CancelOrder'

  export default {
    name: 'PromoteScrollView',
    components: {
      GoodInfoItem,
      CancelOrder,
    },
    ...
    methods: {
      ...
      goOrderDetail(data) {
        // this.$linkToEasy(
        //   '/pagesAgent/Order/OrderDetail/OrderDetail?id=' +
        //     data.id +
        //     '&status=' +
        //     data.status +
        //     '&source=' +
      ...
      async getOrderListRequest() {
        ...
        try {
          let res
          if (this.source === 1) {
            res = await this.$VoHttp.apiOrderSellQueryListPromote(param)
          } else if (this.source === 2) {
            res = await this.$VoHttp.apiOrderSellQueryListInviter(param)
          }
