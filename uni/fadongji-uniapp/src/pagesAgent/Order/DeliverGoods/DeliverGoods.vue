      ...
      <view class="flex1 overflow-y">
        ...
        <view v-for="(item, index) in orderData.goodsDetail" :key="index" class="deliver-goods">
          <view class="p-t-26 flex">
            ...
            <view class="flex1 m-l-16">
              <view class="fz-28 fz-b list-hidden overTwoEllipsis">
              ...
              <view class="good-bottom__num">
                {{ item.goodsCount - item.deliveryCount }}件未发货
              </view>
            </view>
          </view>
          <view class="p-t-24 flex flex-vertical-c">
        ...
        <view class="deliver-info">
          <view class="deliver-info__mode flex flex-vertical-c">
            ...
            <EraRadioGroup
              v-model="radioValue"
              class="deliver-info__group flex-justify-r"
              placement="row"
              @change="groupChange"
            >
              <EraRadio
...
<script>
  ...
  export default {
    ...
    onLoad(option) {
      ...
      //寄售订单 发动机识别码 如果有 显示 否则 不显示】
      this.type = option.type
      if (this.orderData.engineNos) {
        this.engineArr = this.orderData.engineNos
      }
      //地址信息
