  ...
  export default {
    ...
    watch: {
      data: {
        deep: true,
        handler(val) {
          if (val.hasOwnProperty('goodsId') && val.goodsId) {
            this.getData()
          }
        },
      },
    ...
    methods: {
      lookMoreClick() {
        ...
        this.$linkToEasy(
          '/pagesSupplier/InvestmentInfo/InvestEvaluate?goodsId=' + this.info.goodsId,
        )
      },
      getData() {
        //获取好评度及评价总数
        // this.$VoHttp
        //   .apiContentEvaluateQueryProductFavorableComment({
        //     sourceType: 0,
        ...
        //获取好评度及评价总数,获取评价标签
        console.log(this.info.goodsId, '.xxxxxxxxxxxxxxxxxxx')
        this.$VoHttp
          .apiOrderEvaluateFavorableComment({
            sourceId: this.data.goodsId,
          })
          .then((res) => {
            if (res.data) {
              this.evaInfo = res.data
