<template>
  <u-popup v-if="showAlert" :round="8" :show="showAlert" mode="bottom" @close="cancle">
    <view class="type">
      ...
      <view class="type-center">
        <view v-for="(item, index) in typeList" :key="index" @click="typeItem(item)">
          <view :class="item.selected ? 'type-center__selected' : 'type-center__item'">
            <view class="title">{{ item.columnName || item.name }}</view>
          </view>
        </view>
      </view>
    </view>
  ...
  export default {
    ...
    methods: {
      ...
      initialization(selectList) {
        ...
        console.log(this.typeList + '111111')
      ...
      // 选择栏目初始化列表
      initList() {
        this.$VoHttp.apiCreateColumnList({ businessType: this.businessType }).then((res) => {
          this.typeList = res.data
        })
      },
      // // 选择品类初始化列表
      // categoryList() {
      //   this.$VoHttp.goodsCategoryCategorySelect().then((res) => {
      //     this.typeList = res.data
      //   })
      // },
    },
