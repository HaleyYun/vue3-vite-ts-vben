<template>
  <view class="brand flex flex-column overflow-hidden">
    <VoNav is-fixed is-have-more title="选择品牌" />
    <!--  品牌列表：start	-->
    <scroll-view
          ...
          <view class="brand-list-content">
            <view class="hot-list">
              ...
              >
                ...
                <view class="hot-text m-t-16 text-nowrap">
                  {{ b.name }}
                </view>
              </view>
            </view>
  ...
  export default {
    ...
    data() {
      return {
        ...
        hotList: [],
        filterList: [],
        origin: 1, // 来源： 1、供应商上货； 2、供应商修改货品； 3、修理厂首页； 4、修理厂买货列表； 5、修理厂买货店铺主页； 6、服务商首页；
      }
    },
    watch: {
      // 监听字母索引的变化，重新设置尺寸
    ...
    methods: {
      ...
      chooseBrandHandler(item) {
        ...
        const target = {
		  delta: 1,
          url: '/pagesCommon/CarTypeSearch/ChooseCarSeries', // 根据来源则判断路由最终跳转地址
          navData: {
            name: item.name,
            logo: item.logo,
          },
      ...
      async initCarTypeList() {
        try {
          ...
          if (+code === 20001) {
			...
			console.log(this.indexListData, 'xx')
