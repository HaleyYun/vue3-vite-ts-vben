<template>
  <view :class="chatItemInfo.type" class="chat flex flex-justify-between" @click="tapToDetail">
    <view class="chat-left">
      <image :src="chatItemInfo.photoUrl || '/static/default_avatar.png'" class="chat-left__img" />
      <VoBadge
        v-if="chatItemInfo.isDot && chatItemInfo.notReadNum"
        class="chat-left__badge"
        isDot
      />
      <VoBadge
        v-else-if="chatItemInfo.notReadNum"
        :value="chatItemInfo.notReadNum"
        class="chat-left__badge"
        max="99"
      />
      <!-- <VoBadge class="chat-left__badge point" :isDot="true" v-if="chatItemInfo.notReadNum" /> -->
    </view>
    ...
    <view v-else class="chat-right flex1 flex flex-column flex-justify-between">
      ...
      <view class="chat-right__down flex flex-justify-between flex-vertical-c">
        ...
        <view v-if="Array.isArray(chatItemInfo.recordLast)" class="down-msg">
          <block v-if="chatItemInfo.recordLast[0].type">
            {{ chatItemInfo.recordLast[0].content }}
          </block>
          <block v-else>
            {{ chatItemInfo.recordLast[0] }}
          </block>
        </view>
        <view v-else class="down-msg">
          <block v-if="chatItemInfo.recordLast.type || chatItemInfo.recordLast.type === 0">
            {{ chatItemInfo.recordLast.content }}
          </block>
          <block v-else>
            {{ chatItemInfo.recordLast }}
          </block>
        </view>
        <VoIcon v-if="!chatItemInfo.notDisturb" :opacity="0.45" :size="16" name="message-mute" />
      </view>
    </view>
  </view>
</template>
  ...
  export default {
    ...
    methods: {
      tapToDetail() {
        ...
        if (this.chatItemInfo.id === 1) {
          this.$linkToEasy(`${this.chatItemInfo.path}?index=${this.chatItemInfo.recordLast.type}`)
          return
        }
        this.$linkToEasy(this.chatItemInfo.path)
      },
