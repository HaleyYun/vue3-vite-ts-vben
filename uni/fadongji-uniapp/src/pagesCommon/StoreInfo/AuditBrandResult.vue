  ...
  >
    ...
    <view class="result-top flex flex-justify-between" v-else-if="brandInfo.status === 1">
      <view class="result-top__left flex1">
        ...
        <view class="left-time color-block">{{ brandInfo.reason }}</view>
        <view class="left-time">{{ brandInfo.createTime }}</view>
      </view>
    ...
    <view class="result-info">
      ...
      <view class="result-info__con">品牌名称：{{ brandInfo.name }}</view>
      <view class="result-info__con flex">
        <view class="m-r-8">品牌logo：</view>
        <image class="con-logo" :src="brandInfo.logoUrl" />
      </view>
      <view class="result-info__con">申请理由：{{ brandInfo.applicationReason }}</view>
    </view>

    <view class="result-info">
      <view class="result-info__time">申请时间：{{ brandInfo.createTime }}</view>
      <view class="result-info__time">申请人：{{ brandInfo.nickName }}</view>
    </view>
  </view>
</template>
...
<script>
  export default {
    ...
    methods: {
      async initData(id) {
        try {
          const { data } = await this.$VoHttp.apiGoodsBrandApply$Id({ id })
          console.log(data, 'xx')
          this.brandInfo = data
