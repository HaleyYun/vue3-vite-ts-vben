<template>
  <view>
    ...
    >
      <view class="flex flex-vertical-c" @tap.stop="iconClickHandler">
        <slot name="icon">
          <VoIcon v-if="!checked" :color="iconColor" :opacity="opacity" :size="24" name="circle" />
          <VoIcon v-else :color="activeColor" :name="inactiveIcon" :size="24" />
        </slot>
      </view>
      <text
        :style="{
        ...
        class="u-radio__text m-l-8"
        @tap.stop="labelClickHandler"
        >{{ label }}
      </text>
    </view>
    <EraLine v-if="parentData.lineBottom === true" margin="0px 0px 0px 16px" />
  </view>
</template>
...
<script>
   ...
   * @property {String}      opacity    未选中图标透明度
  ...
  export default {
    ...
    computed: {
      ...
      elIconColor() {
        ...
        if (this.elDisabled) {
          ...
          return this.checked ? this.elInactiveColor : ''
        } else {
          return this.checked ? iconColor : ''
        }
      },
      iconClasses() {
        let classes = []
    ...
    methods: {
      ...
      updateParentData() {
        this.getParentData('EraRadioGroup')
      },
      // 点击图标
      iconClickHandler(e) {
        this.preventEvent(e)
...
<style lang="scss" scoped>
  ...
  $u-radio-wrap-border-width: 2px !default;
  $u-radio-wrap-border-color: rgba(0, 0, 0, 0.15) !default;
  ...
  $u-radio-checked-border-color: #22284b !default;
  $u-radio-disabled-background-color: rgba(0, 0, 0, 0.08) !default;
  $u-radio-disabled-border-color: rgba(0, 0, 0, 0.15);
