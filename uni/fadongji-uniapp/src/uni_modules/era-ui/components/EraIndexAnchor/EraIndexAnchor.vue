  ...
  export default {
    ...
    methods: {
      init() {
        ...
        const indexList = uni.$u.$parent.call(this, 'EraIndexList')
        if (!indexList) {
          return uni.$u.error('index-anchor必须要搭配index-list组件使用')
        }
        // 将当前实例放入到u-index-list中
        indexList.anchors.push(this)
        const indexListItem = uni.$u.$parent.call(this, 'EraIndexItem')
        // #ifndef APP-NVUE
        // 只有在非nvue下，u-index-anchor才是嵌套在u-index-item中的
        if (!indexListItem) {
          return uni.$u.error('index-anchor必须要搭配index-item组件使用')
        }
