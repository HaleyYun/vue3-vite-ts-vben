  ...
  export default {
    ...
    data() {
      return {
        ...
        orderTypeList: [
          { name: '普通订单', selected: false, id: '1', time: '1', type: 'month' },
          { name: '预售订单', selected: false, id: '2', time: '1', type: 'month' },
        ],
      }
    },
    methods: {
      orderScree(data) {
        ...
        this.screenNum = String(this.orderNumber + this.orderType)
      },
      resetClick() {
        this.timeData = {}
        this.orderTypeData = {}
        let param = {
          time: [],
          orderTypeData: this.orderTypeData,
          num: '',
        }
        this.orderNumber = 0
        this.orderType = 0
        this.$refs.orderRef?.reset()
        this.$refs.orderTypeRef?.reset()
        this.$emit('screen', param)
      },
      screenClick() {
        if (this.orderNumber === 0 && this.orderType === 0) {
