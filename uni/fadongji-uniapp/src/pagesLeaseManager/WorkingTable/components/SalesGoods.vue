...
<!--          >-->
        ...
        <view class="flex flex-row w-full m-t-24">
          <view class="shopinfo-img">
            <block v-if="item.pic">
              <image
                v-if="item.pic.indexOf('mp4') !== -1"
                :src="item.pic.split('.mp4')[0] + '.mp4&type=3'"
                class="shopinfo-img"
              />
              <image v-else :src="item.pic + '&type=1'" class="shopinfo-img" />
            </block>
...
<!--            <view v-if="item.warning === 2" class="shopinfo-sale">无货</view>-->
          ...
          <view class="flex1 flex flex-column m-l-16">
            ...
            <view class="flex flex-row m-t-16 flex-vertical-c">
              <block v-if="item.cashPrice">
...
<!--              <VoIcon :opacity="0.45" class="m-l-8" name="account-question" size="16" />-->
            ...
            <view
              v-if="item.alreadyInvestedNum && item.totalInvestment"
              class="flex flex-row m-t-8 flex-vertical-c text-center"
            >
              <view class="shopinfo-grey">已投量/总投量</view>
              <view class="color-block-65 fz-28">
                <text>{{ item.alreadyInvestedNum }}</text>
                <text>/</text>
                <text class="fz-24">{{ item.totalInvestment }}</text>
              </view>
            </view>
          </view>
        </view>
  ...
  export default {
    ...
    methods: {
      toGoodsDetail(item) {
        ...
        this.$linkToEasy(`/pagesLeaseManager/WorkingTable/GoodsDetail?id=${item.id}&companyId=${this.companyId}`)
      },
    },
  }
</script>
